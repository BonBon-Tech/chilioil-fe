...
const routes = [
  ...
  },
  {
    ...
    children: [
      ...
      { path: "edit-product/:id", component: Edit_Product },
      { path: "expired-products", component: Expired_Products },
      { path: "low-stocks", component: Low_Stocks },
      { path: "category-list", component: Category_List, meta: { isAuth: true, access: ['admin'] } },
      ...
      { path: "product-details/:id", component: Product_Details },
    ]
  },
  {
    path: '/finance',
  ...
  },
  {
    path: '/dashboard',
    component: Dashboard,
    meta: { isAuth: true, access: ['admin', 'staff'] },
    children: [
      { path: '', redirect: '/dashboard/admin-dashboard' },
      { path: "admin-dashboard", component: AdminDashboard, meta: { isAuth: true, access: ['admin'] } },
      { path: "sales-dashboard", component: SalesDashboard, meta: { isAuth: true, access: ['admin', 'staff'] } },
    ]
  },
  {
    path: '/stock',
  ...
  },
  {
    ...
    children: [
      ...
      { path: "sales-list", component: SalesList, meta: { isAuth: true, access: ['admin'] } },
      { path: "invoice-report", component: InvoiceReport, meta: { isAuth: true, access: ['admin'] } },
...
router.beforeEach((to, from, next) => {
    ...
    const accessRoles = to.matched
        .filter(record => record.meta && record.meta.access)
        .map(record => record.meta.access)
        .flat();

    if (requiresAuth && accessRoles.length > 0) {
        const userRole = user?.role?.name;
        if (!userRole || !accessRoles.includes(userRole)) {
            if (userRole === 'staff') {
                return next({ path: '/sales/pos' });
            }
